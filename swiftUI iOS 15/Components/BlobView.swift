...
struct BlobView: View {
    ...
    @State var apper = false
    var body: some View {
        TimelineView(.animation) { timeLine in
            
            let now = timeLine.date.timeIntervalSinceReferenceDate
            let angele = Angle.degrees(now.remainder(dividingBy: 3) * 60)
            let x = cos(angele.radians)
            let angele2 = Angle.degrees(now.remainder(dividingBy: 6) * 10)
            let x2 = cos(angele2.radians)
            ...
            .rotationEffect(.degrees(apper ? 360 : 0))
    ...
    func path(in rect: CGRect, x:Double , x2:Double) -> Path {
        ...
        path.addCurve(to: CGPoint(x: 0.08995*width*x, y: 0.60171*height), 
